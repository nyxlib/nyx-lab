...
<template>
    ...
    <nav class="navbar navbar-expand nyx-bg1 py-0">
        <div class="container-fluid nyx-bg2 px-2" data-tauri-drag-region>
            ...
            <div class="navbar-brand">
                {{ configStore.globals.windowTitle }}
            </div>
    ...
    <div class="d-flex flex-row nyx-bg1" style="height: calc(100% - 3.0rem); width: 100%;">
        ...
        <ul class="nav nav-pills flex-column overflow-x-hidden overflow-y-scroll nyx-bg2 px-2">
            ...
            <li class="nav-item" title="Control panel" v-tooltip>
                <router-link class="nav-link rounded-1 mb-2" active-class="active" to="/home" style="padding: 0.8rem 1.0rem;">
            ...
            <li class="nav-item" title="Configuration" v-tooltip>
                <router-link class="nav-link rounded-1 mb-2" active-class="active" to="/config" style="padding: 0.8rem 1.0rem;">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 16 16">
            ...
            <template v-for="appPanel in appPanels" :key="appPanel.addon.id">
                <li class="nav-item" :title="panel.title" v-tooltip v-for="(panel, idx) in appPanel.panels" :key="`${appPanel.addon.id}_${idx}`">
                    <router-link class="nav-link rounded-1 mb-2" active-class="active" :to="panel.path" style="padding: 0.8rem 1.0rem;" v-html="panel.logo" />
                </li>
            </template>
            ...
            <template v-for="webPage in webPages" :key="webPage.id">
                <li class="nav-item" :title="webPage.title" v-tooltip v-if="(webPage.url.startsWith('http://') || webPage.url.startsWith('https://')) && webPage.enabled">
                    <router-link class="nav-link rounded-1 mb-2" active-class="active" :to="`/external/${webPage.id}`" style="padding: 0.8rem 1.0rem;" v-html="icons[webPage.icon || 'bi-question']" />
                </li>
            </template>
